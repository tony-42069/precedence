...
export default function OrderBook({ yesOrderBook, noOrderBook }: OrderBookProps) {
  const [selectedToken, setSelectedToken] = useState<'yes' | 'no'>('yes');

  // Get the active order book based on selected token
  const activeOrderBook = selectedToken === 'yes' ? yesOrderBook : noOrderBook;
  const bids = activeOrderBook.bids;
  const asks = activeOrderBook.asks;
